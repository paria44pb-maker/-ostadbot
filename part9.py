...
ماژول هندلرهای اصلی، پنل ادمین، مدیریت کاربران، پرداخت‌ها و VIP
...
async def admin_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """مدیریت کاربران - نمایش منو"""
    stats = db_manager.get_stats() if db_manager else {}
    text = f"""
👥 **مدیریت کاربران**
...
async def admin_users_list(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """لیست کاربران"""
    if get_user_repo:
...
async def admin_payments_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """مدیریت پرداخت‌ها"""
    keyboard = [
    ...
    await query.edit_message_text("💰 **مدیریت پرداخت‌ها**\n\nاز دکمه‌های زیر استفاده کنید:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

async def admin_vip_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """مدیریت VIP"""
    stats = db_manager.get_stats() if db_manager else {}
    text = f"""
💎 **مدیریت VIP**
...
async def admin_broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """ارسال پیام همگانی"""
    keyboard = [
    ...
    await query.edit_message_text("📢 **ارسال پیام همگانی**\n\nمخاطبان خود را انتخاب کنید:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

async def admin_api_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """مدیریت API"""
    keyboard = [
    ...
    await query.edit_message_text("🔧 **مدیریت API**\n\nوضعیت API‌ها:\n✅ Groq AI: فعال\n✅ CoinEx: فعال\n✅ Telegram Bot: فعال", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

async def admin_backup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """بکاپ و بازیابی"""
    keyboard = [
    ...
    await query.edit_message_text("💾 **بکاپ و بازیابی**\n\nاز دکمه‌های زیر برای مدیریت بکاپ‌ها استفاده کنید:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

async def admin_exit_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query):
    """خروج / مدیریت سرور"""
    keyboard = [
    ...
    await query.edit_message_text("🚪 **خروج / مدیریت سرور**\n\n⚠️ هشدار: عملیات‌های زیر غیرقابل بازگشت هستند!", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ====== مدیریت کاربران ======
    if data == "admin_users":
        await admin_users_callback(update, context, query)
        return

    if data == "admin_users_list":
        await admin_users_list(update, context, query)
        return

    # ====== مدیریت پرداخت‌ها ======
    if data == "admin_payments":
        await admin_payments_callback(update, context, query)
        return

    # ====== مدیریت VIP ======
    if data == "admin_vip":
        await admin_vip_callback(update, context, query)
        return

    # ====== ارسال همگانی ======
    if data == "admin_broadcast":
        await admin_broadcast_callback(update, context, query)
        return
    ...
    # ====== مدیریت API ======
    if data == "admin_api":
        await admin_api_callback(update, context, query)
        return

    # ====== بکاپ ======
    if data == "admin_backup":
        await admin_backup_callback(update, context, query)
        return

    # ====== خروج / مدیریت سرور ======
    if data == "admin_exit":
        await admin_exit_callback(update, context, query)
        return
