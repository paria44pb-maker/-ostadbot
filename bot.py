...
"""
╔══════════════════════════════════════════════════════════════════════╗
║   CRYPTO PULSE v14.0 - 50 GLASS KEYS | FULL PERSIAN | IRAN TIMEZONE ║
║   Dual AI | Real Charts | Alanchand Forex | 50+ Active Buttons       ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
# ============================================================
# TIMEZONE + AUTO-INSTALL
# ============================================================
try: import pytz; TEHRAN_TZ = pytz.timezone('Asia/Tehran'); PYTZ_AVAILABLE = True
except:
    try:
        subprocess.check_call([sys.executable,"-m","pip","install","pytz","--quiet"],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
        import pytz; TEHRAN_TZ = pytz.timezone('Asia/Tehran'); PYTZ_AVAILABLE = True
    except: PYTZ_AVAILABLE = False; TEHRAN_TZ = None
...
# ============================================================
# PERSIAN LIVE DATE - IRAN TIMEZONE (همیشه آنلاین)
# ============================================================
class PersianLive:
    ...
    def _now(cls) -> datetime:
        if PYTZ_AVAILABLE and TEHRAN_TZ: return datetime.now(TEHRAN_TZ)
        return datetime.now() + timedelta(hours=3, minutes=30)
    
    @classmethod
    def now(cls) -> datetime: return cls._now()
    @classmethod
    def date_str(cls) -> str: n = cls._now(); return f"{n.day} {cls.MONTHS[n.month-1]} {n.year}"
    @classmethod
    def time_str(cls) -> str: return cls._now().strftime('%H:%M:%S')
    @classmethod
    def day_str(cls) -> str: return cls.DAYS[cls._now().weekday()]
    @classmethod
    ...
    @classmethod
    def tz_info(cls) -> str: return "🇮🇷 Asia/Tehran (ساعت رسمی ایران)" if PYTZ_AVAILABLE else "🇮🇷 UTC+3:30"
...
# ============================================================
# ALANCHAND FOREX - طلا، دلار، یورو، لیر، دینار، پوند، سکه
# ============================================================
...
class Trader:
    ...
    def load(self):
        try:
            with open('trader_v14.json') as f:
    ...
    def save(self):
        try:
            with open('trader_v14.json','w') as f:
...
# ============================================================
# FORMATTER - ULTRA PREMIUM PERSIAN GREEN
# ============================================================
class Fmt:
    @staticmethod
...
# ============================================================
# 50+ GLASS BUTTONS MENU - FULL PERSIAN
# ============================================================
class Menu:
    @staticmethod
    def main() -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📢 ارسال سیگنال به کانال", callback_data="send_signal"),
             InlineKeyboardButton("📚 ارسال آموزش به کانال", callback_data="send_edu"),
             InlineKeyboardButton("📰 ارسال اخبار به کانال", callback_data="send_news")],
        ])
...
class BotInfoUpdater:
    ...
    async def update(self):
        try:
            ...
            try: await bot.set_my_short_description(f"₿ {btc_price} | {pdt.short()} | AI Trading Bot"[:120])
            except: pass
            cmds = [BotCommand("start","🚀 شروع ربات"),BotCommand("signal","🎯 سیگنال BTC"),BotCommand("price","💰 قیمت‌ها"),BotCommand("scan","🔍 اسکن"),BotCommand("portfolio","💼 پورتفوی"),BotCommand("forex","💵 طلا و ارز"),BotCommand("news","📰 اخبار"),BotCommand("edu","📚 آموزش"),BotCommand("chart","📊 نمودار"),BotCommand("help","❓ راهنما")]
            try: await bot.set_my_commands(cmds, scope=BotCommandScopeDefault())
            except: pass
            logger.info(f"🔄 Bio updated: {btc_price} | {pdt.short()}")
        except Exception as e: logger.error(f"Bio error: {e}")
    def start(self):
        def run(): 
...
async def button_router(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if d == "back": await q.edit_message_text(f"🟢 *منو*\n\n📅 {pdt.full()}", parse_mode="Markdown", reply_markup=Menu.main())
            ...
            await signal_handler(update, ctx)
            await q.answer("✅ سیگنال ارسال شد")
...
async def main():
    ...
    logger.info("="*50)
    logger.info(f"🚀 کریپتو پالس ۱۴ | {pdt.full()}")
    ...
    logger.info(f"🔘 ۵۰+ کلید شیشه‌ای فعال")
    logger.info("="*50)
