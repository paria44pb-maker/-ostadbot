...
نسخه کامل با پشتیبانی از فایل token.txt (بدون وابستگی به متغیر محیطی)
"""
...
# ============================================================
# LOAD ENVIRONMENT (اختیاری)
# ============================================================
load_dotenv()

# ============================================================
# TOKEN LOADING - ONLY FROM token.txt (CLEAN)
# ============================================================

def load_token_from_file() -> str:
    """خواندن توکن از فایل token.txt (فقط اولین خط غیرخالی و بدون #)"""
    TOKEN_FILE = "token.txt"
    
    if not os.path.exists(TOKEN_FILE):
        # ایجاد فایل راهنما
        with open(TOKEN_FILE, "w") as f:
            f.write("# Paste your Telegram bot token here (one line, no extra spaces)\n")
    ...
    for line in lines:
        ...
        if not line or line.startswith("#"):
            continue
        # پیدا کردن توکن (با فرمت عدد:حروف)
        if re.match(r'^\d+:[A-Za-z0-9_-]+$', line):
            print(f"✅ Token loaded from {TOKEN_FILE} (length: {len(line)})")
            return line
        else:
            # اگر فرمت نامعتبر بود، اما شاید کاربر اشتباه کرده
            print(f"⚠️ Skipping invalid line: {line[:20]}...")
    
    print("❌ No valid token found in token.txt!")
    print("Please ensure token.txt contains only your token (one line, no extra text).")
    sys.exit(1)
