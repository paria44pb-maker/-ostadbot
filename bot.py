...
<style>*{{margin:0;padding:0}}body{{font-family:system-ui;background:linear-gradient(135deg,#0a0a1a,#1a1a3a,#0d0d2b);color:#e0e0e0;min-height:100vh;display:flex;justify-content:center;align-items:center;padding:20px}}
...
async def full_status():
    return {"bot_ready":bot_ready,"uptime":(datetime.now()-startup_time).total_seconds(),"modules":status,"token":"✅" if BOT_TOKEN else "❌","creator":CREATOR_NAME,"parts_loaded":sum(1 for v in status.values() if "✅" in str(v)),"parts_total":len(status)}
...
def load_parts():
    for pid, name, desc in PARTS:
        try:
            ...
            print(f"   ⚠️ Part {pid:2d} — {desc} — {str(e)[:50]}")
        time.sleep(0.02)
...
def main():
    """Entry point — handles both sync and async contexts"""
    try:
